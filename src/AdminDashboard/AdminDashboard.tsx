...
import { fetchAllEmployeesMonthlyAttendance, AttendanceStatus } from "../reducers/employeeAttendance.reducer";
...
const AdminDashboard = () => {
  ...
  const departments = ["All", "HR", "IT", "Sales", "Marketing", "Finance", "Engineering", "Design", "Admin"];
  ...
  const [exportStep, setExportStep] = useState<'employees' | 'range'>('employees');
  
  const today = new Date();
  const firstDay = new Date(today.getFullYear(), today.getMonth(), 1);
  const lastDay = new Date(today.getFullYear(), today.getMonth() + 1, 0);

  const [exportStartDate, setExportStartDate] = useState(firstDay.toISOString().split('T')[0]);
  const [exportEndDate, setExportEndDate] = useState(lastDay.toISOString().split('T')[0]);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (modalDeptRef.current && !modalDeptRef.current.contains(event.target as Node)) {
  ...
  useEffect(() => {
    dispatch(fetchAllEmployeesMonthlyAttendance({ month: currentMonth, year: currentYear }));
  ...
  const stats = useMemo(() => {
    ...
    if (!allAttendance.length) return { totalHours: 0, todayPresent: 0, totalAbsent: 0 };
    
    const totalMinutes = allAttendance.reduce((acc, curr) => acc + (curr.totalHours || 0) * 60, 0);
    
    const todayRecords = allAttendance.filter(r => {
      const workingDateStr = r.workingDate instanceof Date 
    ...
    const todayPresentCount = todayRecords.filter(r => 
      [AttendanceStatus.FULL_DAY, AttendanceStatus.HALF_DAY].includes(r.status as AttendanceStatus)
    ).length;

    const todayAbsentCount = todayRecords.filter(r => 
  ...
  const employees = entities.map((emp: any) => {
    ...
    return {
      ...
      deptColor: emp.department === "Engineering" ? "#3182CE" : emp.department === "Design" ? "#805AD5" : "#38A169"
    };
  });
  ...
  const filteredModalEmps = entities.filter(emp => {
    const matchesSearch = (emp.fullName || emp.name || "").toLowerCase().includes(modalSearch.toLowerCase());
  ...
  const handleBulkExport = async () => {
    setIsExporting(true);
    try {
      const selected = entities.filter(e => selectedEmps.has(e.employeeId || e.id));
  ...
  const styles = {
    container: "p-4 md:p-8 bg-[#F4F7FE] min-h-screen font-['DM_Sans',sans-serif]",
    cardGrid: "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8",
    statCard: (gradient: string) => `relative overflow-hidden p-6 rounded-[24px] text-white shadow-xl ${gradient} transition-transform hover:-translate-y-1 duration-300`,
    glassInner: "absolute top-[-20px] right-[-20px] w-32 h-32 bg-white/10 rounded-full blur-2xl",
    tableWrapper: "bg-white rounded-[24px] shadow-[0px_18px_40px_rgba(112,144,176,0.08)] overflow-hidden",
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.cardGrid}>
        ...
        <div className={styles.statCard("bg-gradient-to-br from-[#4318FF] to-[#5BC4FF]")}>
          <div className={styles.glassInner}></div>
          <div className="flex justify-between items-start mb-4">
            <h4 className="text-sm font-medium text-white/80">Total Employees</h4>
          ...
          <Users className="absolute bottom-4 right-4 text-white/20" size={48} />
        </div>

        {/* Card 2: Hours Worked */}
        <div className={styles.statCard("bg-gradient-to-br from-[#868CFF] to-[#4318FF]")}>
          <div className={styles.glassInner}></div>
          <div className="flex justify-between items-start mb-4">
            <h4 className="text-sm font-medium text-white/80">Hours Worked</h4>
            <Clock size={20} className="text-white/60" />
          </div>
          <h2 className="text-4xl font-black mb-1">{stats.totalHours.toLocaleString()}</h2>
          <p className="text-[11px] opacity-70 font-bold">+8.5% vs last week</p>
          <Clock className="absolute bottom-4 right-4 text-white/20" size={48} />
        </div>

        {/* Card 3: Today Present */}
        <div className={styles.statCard("bg-gradient-to-br from-[#05CD99] to-[#48BB78]")}>
          <div className={styles.glassInner}></div>
          <div className="flex justify-between items-start mb-4">
          ...
          <TrendingUp className="absolute bottom-4 right-4 text-white/20" size={48} />
        </div>

        {/* Card 4: Total Absent */}
        <div className={styles.statCard("bg-gradient-to-br from-[#FF9060] to-[#FF5C00]")}>
          <div className={styles.glassInner}></div>
          <div className="flex justify-between items-start mb-4">
          ...
          <FileText className="absolute bottom-4 right-4 text-white/20" size={48} />
        </div>
      </div>
              ...
              >
                <td className={styles.td}>
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-bold">{emp.fullName || emp.name}</span>
                  </div>
                </td>
                <td className={`${styles.td} text-center text-gray-400 font-bold`}>{emp.employeeId || emp.id}</td>
                <td className={`${styles.td} text-center`}>
                  <div className="flex items-center justify-center gap-2">
                    <div className="w-2 h-2 rounded-full" style={{ backgroundColor: emp.deptColor }}></div>
                ...
                <td className={`${styles.td} text-center`}>
                  <span className={styles.statusBadge(emp.status)}>{emp.status}</span>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {isExportModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-[#2B3674]/20 backdrop-blur-md" onClick={() => { setIsExportModalOpen(false); setExportStep('employees'); }}></div>
          
          <div className={`relative bg-white w-full transition-all duration-500 ease-in-out rounded-[32px] shadow-2xl overflow-hidden animate-in fade-in zoom-in flex flex-col ${
            exportStep === 'employees' ? 'max-w-3xl h-[720px]' : 'max-w-md h-[480px]'
          }`}>
            {/* Modal Header */}
            <div className="p-8 border-b border-gray-100 flex justify-between items-center bg-white flex-shrink-0">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-2xl font-black text-[#2B3674]">
                    {exportStep === 'employees' ? 'Select Employees' : 'Select Date Range'}
                  </h2>
              ...
              <button 
                onClick={() => { setIsExportModalOpen(false); setExportStep('employees'); }}
              ...
              <div className="flex-1 flex flex-col min-h-0 px-10 py-6">
                ...
                <div className="flex flex-wrap gap-4 items-center flex-shrink-0 mb-8">
                  <div className="relative" ref={modalDeptRef}>
                    ...
                    >
                      <Filter size={16} className="text-[#4318FF]" />
                      <span>{modalDept === "All" ? "All Departments" : modalDept}</span>
                      <ChevronDown size={16} className={`text-[#A3AED0] transition-transform ${isModalDeptOpen ? 'rotate-180' : ''}`} />
                    </button>
                    {isModalDeptOpen && (
                       <div className="absolute top-full left-0 mt-2 w-48 bg-white rounded-2xl shadow-xl border border-gray-100 py-2 z-[110]">
                          {departments.map(d => (
                            <button 
                              key={d} 
                              onClick={() => { setModalDept(d); setIsModalDeptOpen(false); }}
                    ...
                    <thead className="sticky top-0 bg-white z-10">
                      <tr>
                        <th className="pb-2 w-12 text-center font-bold text-[#A3AED0] text-[11px] uppercase tracking-wider">
                          ...
                          >
                            {selectedEmps.size === entities.length && <Check size={14} className="text-white" />}
                          </div>
                        </th>
                        <th className="pb-2 text-left font-bold text-[#A3AED0] text-[11px] uppercase tracking-wider">Employee</th>
                        <th className="pb-2 text-left font-bold text-[#A3AED0] text-[11px] uppercase tracking-wider">Dept</th>
                      </tr>
                    </thead>
                        ...
                        return (
                          ...
                          >
                            <td className="p-4 text-center border-b border-gray-50">
                              <div className={`w-5 h-5 rounded-full border-2 mx-auto flex items-center justify-center transition-all ${isSelected ? 'bg-[#4318FF] border-[#4318FF]' : 'border-gray-200 group-hover:border-[#4318FF]'}`}>
                            ...
                            <td className="p-4 border-b border-gray-50">
                              <div className="flex items-center gap-3">
                                ...
                                <span className="font-bold text-[#2B3674] text-sm">{emp.fullName || emp.name}</span>
                              </div>
                            </td>
                            <td className="p-4 text-sm font-bold text-[#A3AED0] border-b border-gray-50">{emp.department}</td>
                          </tr>
                        );
                      })}
                    </tbody>
                ...
                <div className="p-8 border-t border-gray-100 flex justify-between items-center bg-[#F8F9FC] flex-shrink-0">
                  <span className="text-sm font-bold text-[#A3AED0]">{selectedEmps.size} employees selected</span>
                  <button 
              ...
              <div className="flex-1 flex flex-col px-12 py-12 bg-white min-h-0 justify-center">
                <div className="max-w-md mx-auto w-full flex flex-col gap-8">
                  ...
                  <div className="flex flex-col gap-3">
                    <label className="text-[10px] font-black text-[#A3AED0] uppercase tracking-widest pl-1">From Date</label>
                    <div className="relative group">
                      ...
                      />
                      <Calendar size={18} className="absolute right-4 top-1/2 -translate-y-1/2 text-[#2B3674]/50 pointer-events-none" />
                    </div>
                  </div>

                  {/* To Date */}
                  <div className="flex flex-col gap-3">
                    <label className="text-[10px] font-black text-[#A3AED0] uppercase tracking-widest pl-1">To Date</label>
                    <div className="relative group">
                      <input 
                        type="date"
                        value={exportEndDate}
