...
const EmployeeListView = () => {
  ...
  }>({ key: null, direction: SortDirection.Asc });
  ...
  const employees = entities.map((emp: any) => ({
    ...
    isActive: emp.userStatus !== EmployeeStatus.Inactive,
  }));
  ...
  const confirmToggleStatus = async () => {
    ...
    try {
      const newStatus =
        selectedEmployeeForToggle.userStatus !== EmployeeStatus.Inactive
              ...
              {isDropdownOpen && (
                <div className="absolute top-full left-0 mt-2 w-full sm:w-48 bg-white/80 backdrop-blur-xl rounded-2xl shadow-[0px_20px_40px_rgba(0,0,0,0.1)] border border-white/20 py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                  ...
                  <button
                    key={Department.All}
                    onClick={() => {
                      setSelectedDepartment(Department.All);
                      setIsDropdownOpen(false);
                      setCurrentPage(1);
                    }}
