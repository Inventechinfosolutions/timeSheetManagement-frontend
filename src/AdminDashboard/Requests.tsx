...
import { getAllLeaveRequests, updateLeaveRequestStatus } from "../reducers/leaveRequest.reducer";
...
const Requests = () => {
  ...
  const handleUpdateStatus = async (id: number, status: "Approved" | "Rejected", employeeName: string) => {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case "Work From Home": return <Home size={18} className="text-green-500" />;
      case "Client Visit": return <MapPin size={18} className="text-orange-500" />;
      default: return <Briefcase size={18} className="text-blue-500" />;
    }
  };
  ...
  return (
    <div className="p-4 md:p-8 bg-[#F4F7FE] min-h-screen font-sans">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-[#2B3674]">Employee Requests</h1>
          <p className="text-sm text-gray-500 mt-1">Review and manage all employee work-related requests</p>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-6">
        <div className="relative flex-1 group">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-[#4318FF] transition-colors" size={20} />
          <input
            type="text"
        ...
        <div className="relative">
          ...
          >
            <Filter size={18} className={selectedDept !== "All" ? "text-[#4318FF]" : "text-gray-400"} />
            <span>{selectedDept === "All" ? "All Departments" : selectedDept}</span>
            <ChevronDown size={18} className={`transition-transform duration-300 ${isDeptOpen ? "rotate-180" : ""}`} />
          </button>
            ...
            <>
              ...
              <div className="absolute right-0 mt-3 w-56 bg-white rounded-3xl shadow-2xl border border-blue-50 py-3 z-50 overflow-hidden animate-in fade-in zoom-in duration-200 origin-top-right">
                <div className="px-5 py-2 mb-1">
                  <span className="text-[10px] font-black uppercase tracking-widest text-gray-400">Departments</span>
                </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-[#4318FF] text-white">
                ...
                <th className="px-6 py-4 text-center text-xs font-bold uppercase tracking-wider">Request Type</th>
                <th className="px-6 py-4 text-center text-xs font-bold uppercase tracking-wider">Department</th>
                <th className="px-6 py-4 text-center text-xs font-bold uppercase tracking-wider">Duration</th>
                <th className="px-6 py-4 text-center text-xs font-bold uppercase tracking-wider">Submitted Date</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {loading ? (
                <tr>
                  <td colSpan={7} className="py-20 text-center text-gray-400 font-medium tracking-wide">
                  ...
                  <tr key={req.id} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full bg-blue-50 flex items-center justify-center text-[#4318FF] font-bold text-xs ring-2 ring-blue-50">
                          {req.fullName ? req.fullName.charAt(0) : <User size={20} />}
                        </div>
                        <div>
                          <p className="font-bold text-[#2B3674] leading-tight">{req.fullName || "Unknown"}</p>
                          <p className="text-[10px] text-gray-400 font-bold uppercase tracking-tighter">{req.employeeId}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-center">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <span className="text-sm font-semibold text-[#2B3674]">{req.requestType}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-center">
                    ...
                    <td className="px-6 py-4 text-center">
                      ...
                      <p className="text-[10px] text-[#4318FF] font-black mt-1 uppercase tracking-wider">
                        Total: {req.duration || (dayjs(req.toDate).diff(dayjs(req.fromDate), 'day') + 1)} Day(s)
                      </p>
                    </td>
                    <td className="px-6 py-4 text-center text-sm font-semibold text-[#475569]">
                      {req.submittedDate ? dayjs(req.submittedDate).format("DD MMM - YYYY") : (req.created_at ? dayjs(req.created_at).format("DD MMM - YYYY") : "N/A")}
                    </td>
                    <td className="px-6 py-4 text-center">
                      <span className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase border tracking-wider transition-all ${getStatusColor(req.status)}`}>
                        {req.status}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-center gap-3">
                        ...
                        {req.status === "Pending" && (
                          <>
                            <button 
                              onClick={() => handleUpdateStatus(req.id, "Approved", req.fullName || "Employee")}
                            ...
                            <button 
                              onClick={() => handleUpdateStatus(req.id, "Rejected", req.fullName || "Employee")}
              ...
              <div className="space-y-2">
                ...
                <div className="w-full px-5 py-3 rounded-2xl bg-[#F4F7FE] font-bold text-[#4318FF] flex items-center justify-between">
                  ...
                  <span className="bg-white px-4 py-1 rounded-lg shadow-sm border border-blue-100">
                    {selectedRequest.duration || (dayjs(selectedRequest.toDate).diff(dayjs(selectedRequest.fromDate), 'day') + 1)} Day(s)
                  </span>
                </div>
              </div>
