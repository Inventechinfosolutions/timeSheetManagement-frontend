...
const MobileQuarterlyReviewForm: React.FC<MobileQuarterlyReviewFormProps> = (props) => {
    ...
    const internalHandleConfirmedSubmit = async () => {
        try {
            ...
            if (result?.id) setInternalReviewId(result.id);
            setInternalBackendStatus(result?.status || ReviewStatus.SUBMITTED);
            ...
            if (errStr.includes('already been submitted')) {
                setInternalBackendStatus(ReviewStatus.SUBMITTED);
                message.success('Quarterly review submitted successfully!');
                navigate(`${getBasePath()}/appraisal`, { replace: true });
            } else {
                        ...
                        >
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                {currentStep < TOTAL_STEPS - 1 ? (
                                    ...
                                    >
                                        <span className="flex items-center justify-center gap-1.5">
                                            <span className={currentStep === 0 ? 'inline' : 'hidden sm:inline'}>
                                                Next
                                            </span>
