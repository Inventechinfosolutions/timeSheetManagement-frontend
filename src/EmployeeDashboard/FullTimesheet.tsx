...
import { Clock, ChevronLeft, ChevronRight, Download, X, Calendar as CalendarIcon } from 'lucide-react';
...
const FullTimesheet = ({ entries, calculateTotal, displayDate, onPrevMonth, onNextMonth, scrollToDate, setScrollToDate }: FullTimesheetProps) => {
    ...
    useEffect(() => {
        if (entries.length > 0 && scrollToDate !== null) {
            ...
            if (targetIndex !== -1) {
                ...
                const targetId = isMobile ? `mobile-full-row-${targetIndex}` : `full-row-${scrollToDate}`;
                ...
                if (targetElement) {
                    ...
                    const fallbackElement = document.getElementById(`full-row-${scrollToDate}`) || 
    ...
    const [endDate, setEndDate] = useState(formatDateForInput(lastDay) > todayStr ? todayStr : formatDateForInput(lastDay));
    ...
    const handleDownload = () => {
        ...
        while (tempDate <= end) {
            ...
            const dateStr = tempDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
            const dayName = tempDate.toLocaleDateString('en-US', { weekday: 'long' });

            if (isWeekend) {
                reportData.push([dateStr, dayName, "---", "---", "---", "---", "WEEKEND"]);
            } else {
        ...
        autoTable(doc, {
            startY: 85,
            head: [['Date', 'Day', 'Location', 'Login', 'Logout', 'Total Hours', 'Status']],
            body: reportData,
        ...
        // Save
        const fileName = `Timesheet_${employeeInfo.id}_${employeeInfo.name.replace(/\s+/g, '_')}.pdf`;
        doc.save(fileName);
    ...
    return (
        <div className="flex flex-col min-h-full bg-[#F4F7FE]">
                ...
                <div className="fixed inset-0 z-100 flex items-center justify-center p-4">
                    ...
                    <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-md p-8 animate-in zoom-in-95 duration-300">
                        <div className="flex items-center justify-between mb-8">
                            <h3 className="text-xl font-bold text-[#2B3674]">Download Timesheet</h3>
                            <button 
                                onClick={() => setIsDownloadModalOpen(false)}
                        ...
                        <div className="space-y-6">
                            <div>
                                ...
                                <div className="relative group">
                                    ...
                                    />
                                    <CalendarIcon size={20} className="absolute right-4 top-1/2 -translate-y-1/2 text-[#00A3C4] pointer-events-none transition-transform group-focus-within:scale-110" />
                                </div>
                            </div>
            ...
            <header className="bg-white/70 backdrop-blur-md sticky top-0 z-20 px-4 md:px-8 py-4 md:py-3 flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-gray-100 shadow-sm">
                <h2 className="text-xl md:text-2xl font-bold text-[#2B3674] tracking-tight text-center md:text-left">Full Timesheet</h2>
