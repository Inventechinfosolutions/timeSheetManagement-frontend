...
const SidebarLayout = ({
  ...
  const navItemClass = (isActive: boolean, expanded = true) =>
    [
      ...
      expanded ? "gap-3 px-3 py-2.5" : "lg:justify-center lg:px-0 py-2",
    ].join(" ");
    ...
    <div className="flex flex-col w-full h-screen bg-[#f8f9fa] font-sans text-[#2B3674] overflow-hidden relative">
      ...
      <div className="flex flex-1 min-h-0 relative overflow-hidden">
        ...
        <button
          ...
          className={`mobile-menu-trigger lg:hidden fixed z-1001 left-4 top-[55px] w-11 h-11 bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-gray-100 flex items-center justify-center text-[#4318FF] active:scale-90 transition-all duration-300
            ${
              isMobileOpen
        ...
        <div
          className={`lg:hidden fixed inset-0 bg-[#111c44]/60 backdrop-blur-md z-2000 transition-all duration-500 ease-in-out
            ${
              isMobileOpen
        ...
        <aside
          className={`fixed top-0 lg:absolute lg:top-0 lg:left-0 h-full lg:h-full flex flex-col shrink-0 transition-all duration-300 ease-in-out z-2001 lg:z-30 text-white
            ${
              isMobileOpen
                ? "translate-x-0 w-72"
        ...
        >
          ...
          <div className="lg:hidden flex items-center gap-3 p-6 mb-2 border-b border-white/10">
          ...
          <div className="hidden lg:flex h-14 items-center justify-between px-0 relative mb-0">
            {/* Logo / Title Area */}
            <div
            ...
            {sidebarItems.map((item) => {
              ...
              return (
                <div key={item.name} className="relative group">
                  ...
                  {/* Tooltip */}
                  {!isOpen && !isMobileOpen && (
                    <div className="hidden lg:block absolute left-[calc(100%+10px)] top-1/2 -translate-y-1/2 px-3 py-1.5 bg-[#111c44] text-white text-xs font-bold rounded-lg shadow-xl opacity-0 group-hover:opacity-100 pointer-events-none transition-all duration-200 whitespace-nowrap z-50">
                      {item.name}
        ...
        ${
          isOpen || isMobileOpen
            ? "gap-4 px-4"
            : "lg:justify-center lg:px-0 gap-0"
        }
    `}
              >
