      ...
      >
        ...
        {!isSmall && (
          <div
            className={`flex items-center justify-between gap-2 ${
              isSidebar ? "mb-4" : "flex-col sm:flex-row mb-8 md:gap-4"
            }`}
          >
            <h3
              className={`${
        ...
        <div
          className={`bg-white rounded-xl border border-gray-100 shadow-[0px_4px_20px_rgba(0,0,0,0.05)] ${
            isSmall ? "p-1.5 mb-2" : isSidebar ? "p-2 mb-4" : "p-2 md:p-4 mb-6"
          }`}
        >
          <div
            className={`grid grid-cols-7 ${
              isSmall ? "gap-0.5" : isSidebar ? "gap-1" : "gap-1 md:gap-2"
            }`}
          >
            {daysOfWeek.map((day) => (
              <div
              ...
              <div
                ...
                className={`${
                  isSmall
                    ...
                    : isSidebar
                    ? "h-8 w-8"
                    : "w-[90%] md:w-[85%] lg:w-[80%] h-12 md:h-16 lg:h-20"
                } mx-auto`}
              ></div>
            ))}
            ...
            {monthDays.map((day) => {
              ...
              const isIncomplete =
                entry &&
                (isNotUpdated || entry.status === "Pending") &&
                !entry.isFuture &&
                !entry.isWeekend;
              ...
              return (
                <div
                  ...
                  className={`${
                    isSmall
                      ...
                      : isSidebar
                      ? "w-8 h-8 text-xs"
                      : "w-[90%] md:w-[85%] lg:w-[80%] h-12 md:h-16 lg:h-20 text-xs md:text-sm"
                  } mx-auto rounded-lg flex items-center justify-center font-bold transition-all hover:scale-105 cursor-pointer relative group
                ...
                >
                  ...
                  {!isSmall &&
                    !isSidebar &&
                    entry &&
                    (entry.status || entry.isWeekend) &&
                    (!holiday || (entry.totalHours || 0) > 0) && (
                      <div
