...
const TodayAttendance = ({ setActiveTab }: Props) => {
    ...
    const todayEntry = useMemo(() => {
        const entry = currentMonthEntries.find(e => e.isToday) || currentMonthEntries[0];
        return entry;
    }, [currentMonthEntries]);
    ...
    if (!todayEntry && loading) return (
        <div className="flex items-center justify-center p-20">
            <div className="w-12 h-12 border-4 border-[#00A3C4]/20 border-t-[#00A3C4] rounded-full animate-spin"></div>
        </div>
    );

    if (!todayEntry) return <div className="p-8 text-center text-gray-500">Initializing entry...</div>;
    ...
    return (
        <>
            <header className="bg-[#F4F7FE] md:bg-opacity-50 backdrop-blur-sm sticky top-0 md:relative z-10 px-4 md:px-8 py-5 md:py-5 flex flex-col md:flex-row items-center md:items-center justify-between gap-4 border-b border-gray-100/50 md:border-none">
                <h2 className="text-xl md:text-2xl font-bold text-[#2B3674] tracking-tight text-center md:text-left">Today's Attendance</h2>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                        {todayEntry.status === 'Leave' ? (
                            ...
                            <div className="px-3 md:px-4 py-1.5 md:py-2 bg-[#FFF9E5] text-[#FFB020] rounded-xl md:rounded-full text-[10px] md:text-xs font-bold flex items-center gap-1.5 md:gap-2 uppercase tracking-wide shadow-sm">
                                <Clock className="w-3 md:w-3.5 h-3 md:h-3.5" /> {!todayEntry.loginTime || todayEntry.loginTime === '--:--' ? 'Login Pending' : 'In Progress'}
                            </div>
                            ...
                            <div className="px-3 md:px-4 py-1.5 md:py-2 bg-[#E6FFFA] text-[#01B574] rounded-xl md:rounded-full text-[10px] md:text-xs font-bold flex items-center gap-1.5 md:gap-2 uppercase tracking-wide shadow-sm">
                                <CheckCircle2 className="w-3 md:w-3.5 h-3 md:h-3.5" /> {todayEntry.status.toUpperCase()}
                            </div>
                        )}
                    </div>
                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        ...
                        <div className="bg-[#E9FBF5] rounded-xl p-5 border-l-4 border-[#01B574] relative overflow-hidden group hover:shadow-md transition-all">
                            ...
                            <div className="bg-white rounded-xl px-4 py-2.5 flex items-center justify-between shadow-sm">
                                ...
                                <div className={`w-7 h-7 rounded-full flex items-center justify-center transition-colors ${todayEntry.isSaved ? 'bg-[#E9FBF5] text-[#01B574]' : 'bg-gray-100 text-gray-400'}`}>
                                    {todayEntry.isSaved ? <CheckCircle2 size={16} /> : <Clock size={16} />}
                                </div>
                            </div>
                        </div>

                        {/* Logout Time */}
                        <div className={`rounded-xl p-5 border-l-4 transition-all relative overflow-hidden group hover:shadow-md bg-[#FDF2F2] border-red-500`}>
                            ...
                            <div className="bg-white rounded-xl px-4 py-2.5 flex items-center justify-between shadow-sm">
                                ...
                                <div className={`w-7 h-7 rounded-full flex items-center justify-center transition-colors bg-[#FDF2F2] text-red-500`}>
                                    {todayEntry.isSavedLogout ? <CheckCircle2 size={16} /> : <Clock size={16} />}
                                </div>
                            </div>
                        </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100 flex flex-col justify-between h-full hover:shadow-md transition-all cursor-pointer group">
                        ...
                        <div>
                            <h4 className="text-4xl font-bold text-[#2B3674] mb-1">
                                {currentMonthEntries.filter(e => ['Full Day', 'Half Day', 'WFH', 'Client Visit'].includes(e.status)).length}
                            </h4>
                            <p className="text-gray-500 text-sm font-medium">Total Full Days</p>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100 flex flex-col justify-between h-full hover:shadow-md transition-all cursor-pointer group">
                        ...
                        <div>
                            <h4 className="text-4xl font-bold text-[#2B3674] mb-1">
                                {(() => {
                                    const workedDays = currentMonthEntries.filter(e => 
                                        e.loginTime && e.logoutTime && 
                                        !e.loginTime.includes('NaN') && !e.logoutTime.includes('NaN')
                                    );
                                    if (workedDays.length === 0) return 0;
                                    
                                    const totalMinutes = workedDays.reduce((acc, curr) => {
                                        const loginMins = curr.loginTime.includes(' ') ? 
                                            (() => {
                                                const [t, p] = curr.loginTime.split(' ');
                                            ...
                                            (() => {
                                                const [h, m] = curr.loginTime.split(':').map(Number);
                                                return h * 60 + m;
                                            })();

                                        const logoutMins = curr.logoutTime.includes(' ') ? 
                                            (() => {
                                                const [t, p] = curr.logoutTime.split(' ');
                                                let [h, m] = t.split(':').map(Number);
                                                if (p === 'PM' && h !== 12) h += 12;
                                                else if (p === 'AM' && h === 12) h = 0;
                                                return h * 60 + m;
                                            ...
                                            (() => {
                                                const [h, m] = curr.logoutTime.split(':').map(Number);
                                                return h * 60 + m;
                                            })();
                                    ...
                                    return Math.round(totalMinutes / (workedDays.length * 60)) || 0;
                                })()}
                            </h4>
                            <p className="text-gray-500 text-sm font-medium">Average working hours</p>
                        </div>
                    ...
                    >
                        ...
                        <div>
                            <h4 className="text-4xl font-bold text-[#2B3674] mb-1">
                                {currentMonthEntries.filter(day => day.status === 'Not Updated').length}
                            </h4>
