...
import "./LeaveManagement.tab.css";
import {
  Home,
  MapPin,
  X,
...
const LeaveManagementTab: React.FC<LeaveManagementTabProps> = ({
  ...
  // Hide the mobile hamburger trigger while any leave modal is open (safe no-op on tablet/desktop)
  useEffect(() => {
  ...
  // Reset the Application Modal's scroll position every time it opens or switches type
  useEffect(() => {
    if (isModalOpen && modalBodyRef.current) {
      modalBodyRef.current.scrollTop = 0;
    }
  }, [isModalOpen, selectedLeaveType]);
  return (
    <div className="leave-management-tab">
      <div className="leave-management-page p-4 md:px-8 md:pb-0 md:pt-0 bg-[#F4F7FE] h-full max-h-full overflow-hidden flex flex-col font-sans text-[#2B3674]">
        ...
        <div className="sticky top-0 z-40 bg-[#F4F7FE] -mx-4 px-4 py-2 mb-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 transition-all">
          ...
          <div className="leave-action-buttons flex gap-2.5 shrink-0">
            {LEAVE_APPLY_OPTIONS.map((option, idx) => (
              <button
                key={idx}
                onClick={() => handleOpenModal(option.label)}
                className={`group relative overflow-hidden border ${option.border}
      bg-linear-to-br ${option.cardBg}
      p-2 rounded-xl shadow-sm hover:shadow-md
      hover:-translate-y-0.5 transition-all duration-200
      flex flex-col items-center justify-center
      gap-1 h-[60px] w-[100px] cursor-pointer`}
              >
                <div
                  className={`w-7 h-7 rounded-lg flex items-center justify-center
    ${option.iconBg} shadow-sm shrink-0`}
                >
                  <option.icon
                    className={`text-white ${option.label === WorkLocation.WORK_FROM_HOME ? "wfh-action-icon" : ""}`}
                    strokeWidth={2}
                  />
                </div>

                <span className="text-[#2B3674] font-bold text-[9px] leading-tight text-center">
                  {option.label}
                </span>
              </button>
            ))}
        ...
        <div className="flex-1 overflow-y-auto pr-1 custom-scrollbar pb-6 space-y-4">
          ...
          <div className="mt-5 mb-3">
            ...
            <div className="flex flex-wrap gap-3 items-end">
              ...
              <div className="flex flex-col gap-1.5 flex-[1.4] min-w-[240px]">
                <span className="text-xs font-bold text-gray-400 pl-1">
                  Status
                </span>
              ...
              {(selectedMonth !== "All" ||
                ...
                <button
                  ...
                  className="h-10 self-end flex items-center justify-center gap-2 px-4 bg-[#5B4FFF] text-white rounded-full hover:bg-[#4318FF] transition-all text-sm font-bold whitespace-nowrap"
                >
                  <X size={16} />
                  Clear All
                </button>
