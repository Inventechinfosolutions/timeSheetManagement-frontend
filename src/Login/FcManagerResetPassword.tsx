...
const FcManagerResetPassword: React.FC = () => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!isValidPassword || !passwordsMatch) {
            setLocalError('Please ensure all requirements are met and passwords match.');
            return;
        }
    ...
    return (
        <div className="min-h-screen bg-[#f8f9fc] flex flex-col font-sans">
                    ...
                    <div className="w-full max-w-lg">
                        {success ? (
                            <div className="bg-white rounded-[2rem] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.06)] p-12 text-center animate-in zoom-in-95 duration-500">
                                ...
                                <h2 className="text-3xl font-extrabold text-[#2B3674] mb-4">Success!</h2>
                                <p className="text-gray-500 font-medium mb-6">Password details updated successfully.</p>
                                <button
                                ...
                                <p className="text-blue-400 text-xs font-semibold animate-pulse">Redirecting in a few seconds...</p>
                            </div>
                        ) : (
                            <div className="bg-white rounded-3xl shadow-[0_30px_80px_rgba(0,0,0,0.08)] border border-gray-100 p-10 md:p-14 relative overflow-hidden group">
                                ...
                                <div className="text-center mb-10">
                                    ...
                                    <p className="text-[#A3AED0] font-medium text-lg max-w-xs mx-auto">
                                        {isForgotMode ? 'Create a secure password to complete your account setup.' : 'Please update your password to continue.'}
                                    </p>
                                </div>
                                    ...
                                    >
                                        ...
                                        {isLoading ? (
                                            <>
                                                ...
                                                {isForgotMode ? 'Setting Password...' : 'Updating Password...'}
                                            </>
                                        ) : (
                                            isForgotMode ? 'SET PASSWORD' : 'UPDATE PASSWORD'
                                        )}
                                    </button>

                                    <div className="bg-gray-50/50 rounded-2xl p-6 border border-gray-100/50">
                                        <p className="text-sm font-bold text-[#2B3674] mb-3">Note:</p>
                                        <ul className="space-y-2.5">
                                            {requirements.map((req, index) => (
                                                <li
                                                    key={index}
                                                    className={`text-[12px] font-bold flex items-start gap-2 transition-all ${
                                                        password ? (req.met ? 'text-green-600' : 'text-red-500') : 'text-gray-400'
                                                    }`}
                                                >
                                                    <span className="mt-0.5 opacity-80">{index + 1}.</span>
            ...
            <footer className="h-14 bg-white border-t border-gray-100 flex items-center justify-center px-6 text-[11px] font-bold text-gray-400 gap-2">
                Copyright © - 
                <img src={inventechLogo} alt="Keonics" className="h-4 grayscale opacity-50" />
                Karnataka State Electronics Development Corporation Limited | Designed and Developed by <span>Inventech Info Solutions</span>
            </footer>
        </div>
    );
};
